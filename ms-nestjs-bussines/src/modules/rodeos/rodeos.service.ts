...
export class RodeosService {
  ...
  async getEstadisticas(
    ...
    // ✅ CORRECCIÓN: Usar 'Vivo' y 'Muerto' con mayúscula
    const stats = await this.rodeosRepository.query(
      `
  SELECT 
    COUNT(*) as total,
    COUNT(CASE WHEN estado::text = 'Vivo' THEN 1 END) as vivos,
    COUNT(CASE WHEN estado::text = 'Muerto' THEN 1 END) as muertos,
    AVG(peso_nacer) as peso_promedio
  FROM terneros 
  WHERE id_rodeo = $1
  `,
      [id],
