...
const ListadoMadre = () => {
  ...
  const eliminarMadre = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/madres/delete-madre-by-id/${modalEliminar.madre.id_madre}${userPayload?.rol === "admin" && establecimientoActual ? `?id_establecimiento=${establecimientoActual}` : ""}`,
        {
          method: "DELETE",
          headers: {
  ...
  useEffect(() => {
    cargarMadresList();
  }, [establecimientoActual]); // ⬅️ DEPENDENCIA ACTUALIZADA
      ...
      <div className='relative flex flex-col w-full h-full overflow-scroll text-slate-300 bg-slate-800 shadow-lg rounded-xl p-3 sm:p-4 md:p-6'>
        ...
        <div className='mt-4 sm:mt-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4'>
          <div className='bg-slate-700 p-3 sm:p-4 rounded-lg text-center'>
            ...
            <p className='text-xs sm:text-sm text-slate-400'>Total Madres</p>
          </div>
          <div className='bg-slate-700 p-3 sm:p-4 rounded-lg text-center'>
            <h3 className='text-xl sm:text-2xl font-bold text-green-400'>
              {madres.filter((m) => m.estado === "Seca").length}
            </h3>
            <p className='text-xs sm:text-sm text-slate-400'>Madres Secas</p>
          </div>
          <div className='bg-slate-700 p-3 sm:p-4 rounded-lg text-center'>
