    ...
    try {
      ...
      if (!createTerneroDto.id_establecimiento) {
        throw new ForbiddenException(
          'No se puede crear un ternero sin establecimiento asignado',
        );
      }
  ...
  try {
    ...
    if (esAdmin) {
      ...
      if (idEstablecimientoQuery) {
        console.log('✅ Admin filtrando por establecimiento:', idEstablecimientoQuery);
        query.where('ternero.id_establecimiento = :idEstablecimiento', {
          idEstablecimiento: idEstablecimientoQuery,
        });
      } else {
        console.log('✅ Admin viendo TODOS los establecimientos (sin filtro)');
        // No agregar ningún where - devuelve todo
      }
    } else {
      // Si NO es admin, SIEMPRE filtrar por su establecimiento
      if (idEstablecimiento) {
        console.log('✅ Usuario no-admin, filtrando por su establecimiento:', idEstablecimiento);
        query.where('ternero.id_establecimiento = :idEstablecimiento', {
          idEstablecimiento,
        });
      } else {
    ...
    const terneroList = await query.getMany();
...
Después de aplicar estos cambios, los logs deberían verse así:

### **Caso 1: Admin SIN seleccionar establecimiento**
```
🔍 Controller - ID del usuario: null Es Admin: true
📥 Query Param recibido: null
🔍 Service findAll - ID Usuario: null Es Admin: true Query Param: null
✅ Admin viendo TODOS los establecimientos (sin filtro)
✅ Encontrados 7 terneros
```

### **Caso 2: Admin selecciona Establecimiento Sur (id=2)**
```
Incoming Request...
URL: /terneros/obtener-listado-terneros?id_establecimiento=2

🔍 Controller - ID del usuario: null Es Admin: true
📥 Query Param recibido: 2
🔍 Service findAll - ID Usuario: null Es Admin: true Query Param: 2
✅ Admin filtrando por establecimiento: 2
✅ Encontrados 4 terneros
```

### **Caso 3: Veterinario Sur**
```
🔍 Controller - ID del usuario: 2 Es Admin: false
📥 Query Param recibido: null
🔍 Service findAll - ID Usuario: 2 Es Admin: false Query Param: null
✅ Usuario no-admin, filtrando por su establecimiento: 2
✅ Encontrados 4 terneros
