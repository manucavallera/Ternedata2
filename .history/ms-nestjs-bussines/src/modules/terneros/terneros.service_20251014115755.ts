    ...
    try {
      ...
      const madre = await this.madreRepository.findOne({
        where: {
          ...
          id_establecimiento: createTerneroDto.id_establecimiento, // 🆕 Validar mismo establecimiento
        },
      });
      ...
      nuevoTernero.id_establecimiento = createTerneroDto.id_establecimiento; // 🆕 AGREGADO
      ...
      console.log('Ternero creado con establecimiento:', {
  ...
  async findAll(
    ...
    try {
      ...
      return terneroList.map((ternero, index) => {
        try {
          console.log(
            `Procesando ternero ${index + 1} - ID: ${ternero.id_ternero}`,
          );
      ...
      if (updateTerneroDto.id_madre) {
        const nuevaMadre = await this.madreRepository.findOne({
          where: {
            ...
            id_establecimiento: ternero.id_establecimiento, // 🆕 Validar mismo establecimiento
          },
        });
