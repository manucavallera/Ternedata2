...
export class TratamientosService {
  ...
  async findAll(
    ...
    try {
      ...
      console.log('🔍 findAll tratamientos:', {
        idEstablecimiento,
        esAdmin,
        tipo_enfermedad,
        turno,
      ...
      if (!esAdmin && idEstablecimiento) {
        console.log('✅ Filtrando por establecimiento:', idEstablecimiento);
        queryBuilder.andWhere(
          'tratamiento.id_establecimiento = :idEstablecimiento',
          { idEstablecimiento },
        );
      } else {
        console.log('⚠️ NO se filtra por establecimiento. esAdmin:', esAdmin);
      }

      const tratamientosArray = await queryBuilder.getMany();
      console.log('📊 Tratamientos encontrados:', tratamientosArray.length);
  ...
  async findAll(
    ...
    try {
      // 👇 DEBUG COMPLETO
      console.log('🔍 ============ DEBUG TRATAMIENTOS ============');
      console.log('📥 Parámetros recibidos:', {
        idEstablecimiento,
        esAdmin,
        tipo_enfermedad,
        turno,
        turno_type: typeof turno,
        turno_value: turno ? `"${turno}"` : 'undefined',
      });
      ...
      if (turno) {
        ...
        console.log('🕐 Turno comparación:', { turno, type: typeof turno });
        queryBuilder.andWhere('tratamiento.turno = :turno', { turno });
      } else {
        console.log('⚠️ NO hay filtro de turno (turno es:', turno, ')');
      }
