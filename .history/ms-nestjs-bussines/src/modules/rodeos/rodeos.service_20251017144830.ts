...
export class RodeosService {
  ...
  async findAll(
    ...
    // ⬅️ Cambiar a any[] en lugar de Rodeos[]
    ...
    if (esAdmin) {
      ...
      // Si es admin y no hay query, ve todos
    } else {
      // No admin solo ve su establecimiento
    ...
    // ⬅️ AGREGAR CONTADOR DE TERNEROS MANUALMENTE
    const rodeosConConteo = await Promise.all(
      rodeos.map(async (rodeo) => {
