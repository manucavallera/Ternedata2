...
async function bootstrap() {
  ...
  const allowedOrigins = [
    'http://localhost:3000',
    'http://localhost:3002', // 👈 ¡AGREGADO! Aquí corre tu Frontend Next.js
    'http://localhost:5173',
  ...
  app.enableCors({
    origin: (origin, callback) => {
      // Permite llamadas sin origin (Postman, Thunder Client, Mobile Apps)
      ...
      console.error(`❌ CORS bloqueó origen: ${origin}`); // Log útil para debug
