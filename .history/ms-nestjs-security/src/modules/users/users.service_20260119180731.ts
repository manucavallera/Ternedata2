...
export class UsersService {
  ...
  async findOne(id: number): Promise<UserEntity> {
    const user = await this.usersRepository.findOne({
      where: { id },
      // 👇 Agregamos ambas relaciones aquí también por si acaso
      relations: ['establecimiento', 'userEstablecimientos'],
      // Comenté el select temporalmente para asegurar que traiga todo,
      // puedes descomentarlo si tu entity 'Establecimiento' está bien exportada
      /*
      select: [
        'id', 'name', 'email', 'rol', 'estado', 'telefono', 
        'id_establecimiento', 'fecha_creacion', 'ultimo_acceso'
      ],
      */
    });
