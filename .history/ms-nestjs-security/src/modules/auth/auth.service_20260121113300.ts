...
export class AuthService {
  ...
  async register(registerAuthDto: RegisterAuthDto) {
    ...
    const userObject = {
      ...
      // 👆👆👆 Si hay invitación, usa su rol. Si no, operario.
  ...
  crearTokenMagico(emailRecibido: string, rolRecibido: string) {
    // 👈 Recibe rol
    const payload = {
      email: emailRecibido,
      id_establecimiento: 10,
      rol: rolRecibido, // 👈 Usa el rol que le pasas
    };

    const token = this.jwtService.sign(payload);

    return {
      instruccion: `Token para ${emailRecibido} con rol ${rolRecibido}`,
