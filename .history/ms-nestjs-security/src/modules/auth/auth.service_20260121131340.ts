...
export class AuthService {
  ...
  async register(registerAuthDto: RegisterAuthDto) {
    ...
    if (invitationToken) {
      try {
        ...
        console.log(
          ...
          ' Rol:',
          datosInvitacion.rol, // 👈 Log para ver qué rol trae
        );
      } catch (error) {
    ...
    const userObject = {
      ...
      // 🔴 ANTES: id_establecimiento: null,
      // 🟢 AHORA: ¡Guardamos el ID de la invitación directamente en el usuario!
    ...
    if (datosInvitacion) {
      try {
        await this.userEstablecimientoRepository.save({
          ...
          // Asegúrate que aquí también respete el rol
  ...
  crearTokenMagico(
    ...
    const payload = {
      email: emailRecibido,
      // 👇 Si viene ID úsalo, si no, usa null (ya no hardcodeamos el 10 siempre)
    ...
    return {
      instruccion: `Token para ${emailRecibido} (Rol: ${rolRecibido}, Granja: ${idEstablecimiento})`,
