...
import { useRouter } from "next/navigation"; // ✅ CORRECTO en Next.js 13+
...
function Navbar() {
  ...
  const router = useRouter(); // ✅ Ahora funciona correctamente
  ...
  return (
    <nav className="flex justify-between items-center bg-green-400 text-white px-24 py-3 relative">
      {status === "checking" || (status !== "authenticated" && statusSession === false) ?
      (
      ...
      (
        <>    {/* Menú para realizar su tarea */}
          <button onClick={onclickIngreso} className="text-x font-bold flex items-center hover:text-green-500">
            <svg
          ...
          <button onClick={onclickListado} className="text-x font-bold flex items-center hover:text-green-500">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
      ...
      {/* Sección de usuario acceso y registro*/}
      <ul className="flex gap-x-4">
        {status === "checking" || (status !== "authenticated" && statusSession === false) ? (
          ...
          <>
              <h1 onClick={onClickLogOut}>{`${userPayload?.name}`}</h1>
            {/* Dropdown de perfil */}
            <li className="relative">
              ...
              {isProfileOpen && (
                <div
                  ...
                  onMouseEnter={() => setIsProfileOpen(true)}  // Abrir el menú cuando el mouse entra
                  onMouseLeave={() => setIsProfileOpen(false)} // Cerrar el menú cuando el mouse sale
                >
                  <button
