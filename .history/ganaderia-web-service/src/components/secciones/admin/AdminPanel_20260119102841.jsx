...
const AdminPanel = () => {
  ...
  useEffect(() => {
    // 1. Identificamos el ID actual (manejamos si Redux guarda el objeto entero o solo el ID número)
    const idTarget =
      establecimientoActual?.id_establecimiento || // Si es objeto
      establecimientoActual || // Si es solo número
      userPayload?.id_establecimiento;

    // 2. Buscamos ese ID en la lista fresca que trajimos del backend
    if (idTarget && establecimientos.length > 0) {
      const estEncontrado = establecimientos.find(
        (e) => e.id_establecimiento == idTarget,
      );

      if (estEncontrado && estEncontrado.configuracion) {
        console.log(
          "📥 Configuración fresca cargada:",
          estEncontrado.configuracion,
        );

        setConfigData((prev) => ({
          ...prev,
          umbralMortalidad: estEncontrado.configuracion.umbral_mortalidad || 5,
          umbralMorbilidad: estEncontrado.configuracion.umbral_morbilidad || 10,
        }));
      }
    }
  }, [establecimientos, establecimientoActual, userPayload]); // 👈 Importante: Reacciona cuando cargan los establecimientos
  ...
  // ✅ FUNCIÓN CORREGIDA: Actualizar Perfil (PUT + ID)
  const handleUpdateProfile = async (e) => {
    e.preventDefault();
    console.log("👤 Actualizando perfil...", configData);
    ...
    // 2. Obtener ID del usuario (seguro)
    const userId = userPayload?.id || userPayload?.sub;
    if (!userId) {
      window.alert("⚠️ Error: No se encuentra el ID del usuario. Recarga la página.");
      return;
    }
    ...
    try {
      ...
      // Solo enviar password si la escribió
      ...
      // 4. 🔥 CAMBIO CLAVE: Usamos PUT y la URL con ID
      ...
      // 5. Actualizar Redux (cambia el nombre en la barra superior al instante)
      if (response.data) {
    ...
    if (!userId) {
      window.alert(
        "❌ Error: No se pudo identificar al usuario. Recarga la página.",
      );
      return;
    }
    ...
    try {
      ...
      // 3. 🔄 CAMBIO AQUÍ: Usamos la ruta con ID
      // Probamos PATCH a '/users/:id' que es lo normal en NestJS
      ...
      // Tip extra: Si PATCH falla con 404, a veces los backends viejos solo aceptan PUT
      if (error.response?.status === 404) {
        window.alert(
          "❌ Error: Ruta no encontrada. Tu backend podría requerir PUT en lugar de PATCH.",
        );
      } else {
        window.alert("❌ Error al actualizar. Revisa la consola.");
      }
    }
  };
  ...
  const handleSaveBusinessRules = async (e) => {
    e.preventDefault();
    console.log("🖱️ Botón presionado: Guardando reglas...");

    // 1. Obtener ID seguro (evita errores de undefined)
    const idEstablecimiento =
      establecimientoActual?.id_establecimiento ||
      userPayload?.id_establecimiento;

    if (!idEstablecimiento) {
      // 👇 USAMOS window.alert PARA EVITAR CONFLICTOS
      window.alert("⚠️ Error: No hay un establecimiento seleccionado.");
      return;
    }

    try {
      console.log(`📡 Enviando datos al ID ${idEstablecimiento}:`, configData);

      const payload = {
        configuracion: {
          umbral_mortalidad: parseInt(configData.umbralMortalidad),
          umbral_morbilidad: parseInt(configData.umbralMorbilidad),
        },
      };

      // 2. Usar el Hook (que ya maneja la URL correcta al puerto 3000)
      await actualizarEstablecimientoHook(idEstablecimiento, payload);

      // 👇 ÉXITO
      window.alert("✅ ¡Éxito! Reglas de negocio actualizadas correctamente.");

      await cargarEstablecimientos();

      // Opcional: Recargar para ver los cambios reflejados en las tarjetas
      // window.location.reload();
    } catch (error) {
      console.error("❌ Error guardando reglas:", error);
      // 👇 ERROR
      window.alert("❌ Hubo un error al guardar. Revisa la consola (F12).");
    }
  };
