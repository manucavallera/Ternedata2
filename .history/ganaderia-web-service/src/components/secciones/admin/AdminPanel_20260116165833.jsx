...
const AdminPanel = () => {
  ...
  const { establecimientoActual } = useSelector(
    (state) => state.business || {} // ✅ Corregido a "business" (como en store.js)
  );
  ...
  const [usuarioSeguro, setUsuarioSeguro] = useState(
    userPayload?.id && userPayload?.name !== "USUARIO" ? userPayload : null
  );
  ...
  // 👇 NUEVO: Cargar datos al entrar a la pestaña configuración
  useEffect(() => {
    if (activeTab === "configuracion" && usuarioSeguro) {
      // ✅ CAMBIO A usuarioSeguro
      setConfigData((prev) => ({
        ...prev,
        name: usuarioSeguro.name || "", // ✅ CAMBIO
        email: usuarioSeguro.email || "", // ✅ CAMBIO
        telefono: usuarioSeguro.telefono || "", // ✅ CAMBIO
        // ... resto igual
      }));
    }
  }, [activeTab, usuarioSeguro]); // ✅ AGREGAR usuarioSeguro a las dependencias
  ...
  const handleSaveBusinessRules = (e) => {
    e.preventDefault();
    // Aquí conectaremos con MS-BUSINESS más adelante
    localStorage.setItem(
      "reglas_negocio",
      JSON.stringify({
        mortalidad: configData.umbralMortalidad,
        morbilidad: configData.umbralMorbilidad,
      })
    );
    showAlert("Reglas de alerta actualizadas (Local)", "success");
  };
  ...
  // Modificar este useEffect en AdminPanel.jsx
  ...
  useEffect(() => {
    ...
    const recuperarUsuario = async () => {
      ...
      try {
        // 🚀 SIEMPRE pedimos los datos frescos al servidor al iniciar
        // Esto garantiza que si cambiaste de rol en la BD, se refleje al instante
        const response = await axios.get(`${API_URL}/users/profile/me`, {
          headers: { Authorization: `Bearer ${token}` },
        });

        console.log("👤 Datos frescos del servidor:", response.data);
        setUsuarioSeguro(response.data);

        // Opcional: Si usas Redux, aquí podrías actualizarlo también
    ...
    // ⚠️ IMPORTANTE: Dejamos el array de dependencias VACÍO []
    // Esto evita que Redux sobrescriba los datos después de cargar
  }, []);
