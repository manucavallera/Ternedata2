...
const AdminPanel = () => {
  ...
  useEffect(() => {
    // 1. Si Redux ya despertó y tiene un ID real, lo usamos
    if (userPayload && userPayload.id && userPayload.name !== "USUARIO") {
      setUsuarioSeguro(userPayload);
      return;
    }

    // 2. Si no, lo buscamos en el servidor
    const recuperarUsuario = async () => {
      const token = getToken();
      if (!token) {
        router.push("/"); // Si no hay token, al login directo
        return;
      }
      ...
      try {
        ...
        // ¡Aquí es donde se quita el loading y aparece el panel!
        setUsuarioSeguro(response.data);
      } catch (error) {
        console.error("Error recuperando usuario", error);
        handleLogout(); // Si falla (token vencido), sacar al usuario
      }
    };
