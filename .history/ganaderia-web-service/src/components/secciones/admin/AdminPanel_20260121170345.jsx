...
const AdminPanel = () => {
  ...
  useEffect(() => {
    const cargarPerfil = async () => {
      ...
      try {
        ...
        // Si NO es admin ni super_admin, lo sacamos volando
        if (
          usuarioFresco.rol !== "admin" &&
          usuarioFresco.rol !== "super_admin"
        ) {
          alert("⛔ Acceso Denegado: Área restringida a Administradores.");
          router.push("/"); // Lo mandamos al inicio (o login)
  ...
  const handleUpdateProfile = async (e) => {
    ...
    if (!userId) {
      window.alert(
        "⚠️ Error: No se encuentra el ID del usuario. Recarga la página.",
      );
      return;
    }
    ...
    try {
      ...
      window.alert("❌ Error al actualizar. Revisa la consola (F12).");
    }
  };
