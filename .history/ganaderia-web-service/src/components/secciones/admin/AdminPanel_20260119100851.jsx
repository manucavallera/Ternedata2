...
const AdminPanel = () => {
  ...
  const handleSaveBusinessRules = async (e) => {
    ...
    if (!idEstablecimiento) {
      alert("⚠️ Error: No hay un establecimiento seleccionado."); // 👈 Cambiado a alert
      return;
    }
    ...
    try {
      ...
      // 2. Usar el Hook (que ya apunta al puerto 3000 correctamente)
      await actualizarEstablecimientoHook(idEstablecimiento, payload);

      alert("✅ ¡Éxito! Reglas de negocio actualizadas."); // 👈 Cambiado a alert

      // Opcional: Recargar la página para ver cambios en el Dashboard
      // window.location.reload();
    } catch (error) {
      console.error("❌ Error guardando reglas:", error);
      alert("❌ Hubo un error al guardar. Revisa la consola."); // 👈 Cambiado a alert
    }
  };
