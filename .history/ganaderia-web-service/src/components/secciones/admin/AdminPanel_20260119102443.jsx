...
const AdminPanel = () => {
  ...
  // ✅ VERSIÓN DEFINITIVA: Actualizar Perfil
  const handleUpdateProfile = async (e) => {
    e.preventDefault();
    console.log("👤 Actualizando perfil...", configData);

    // 1. Validación de contraseñas (solo si escribió algo)
    ...
    try {
      ...
      const payload = {
        ...
        email: configData.email, // Agregamos email por si lo permite tu backend
        telefono: configData.telefono,
      };

      // Solo agregamos password si realmente la quiere cambiar
      if (configData.newPassword) {
        payload.password = configData.newPassword;
      }

      // 3. Enviamos al puerto 3001 (API_URL)
      // Usamos PATCH y la ruta '/users/profile/me' que es la estándar para "mi perfil"
      ...
      // 4. 🔥 MAGIA: Actualizamos Redux para ver el cambio YA MISMO en la barra superior
      if (response.data) {
      ...
      window.alert(
        "❌ Error al actualizar. Verifica si el email ya existe o intenta más tarde.",
      );
    }
  };
