...
const AdminPanel = () => {
  ...
  // Si userPayload no tiene ID real, empezamos en null para forzar la carga
  ...
  // 👇 2. PEGAR ESTO: Recuperar identidad si recargas la página
  useEffect(() => {
    // ✅ CORRECCIÓN: Solo confiamos si tiene un ID real
    if (userPayload && userPayload.id) {
      setUsuarioSeguro(userPayload);
      return;
    }

    // Si no, intentar recuperarlos del Backend usando el token guardado
    const recuperarUsuario = async () => {
      const token = getToken();
      if (!token) return;

      try {
        // Le preguntamos al backend: "¿Quién soy?"
        const response = await axios.get(`${API_URL}/users/profile/me`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        setUsuarioSeguro(response.data); // ¡Memoria recuperada!
      } catch (error) {
