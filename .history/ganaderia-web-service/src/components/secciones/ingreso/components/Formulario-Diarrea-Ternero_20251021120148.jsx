...
import { useDispatch } from "react-redux"; // Ya existe useSelector
...
const FormularioDiarreaTernero = ({ setStep }) => {
  ...
  useEffect(() => {
    // Cargar desde localStorage si Redux está vacío
    if (!userPayload || Object.keys(userPayload).length === 0) {
      ...
      if (userSelected && token) {
        try {
          ...
          // Importar estas acciones al inicio del archivo
          dispatch(setUserData(userData));
          dispatch(setAuthPayload(token));
          dispatch(setStatus("authenticated"));
        } catch (error) {
          console.error("❌ Error parseando userSelected:", error);
        }
      }
    }
  }, []);
  ...
  const onSubmit = async () => {
    ...
    console.log("📤 Enviando diarrea:", newDiarreaTernero); // Para debug
