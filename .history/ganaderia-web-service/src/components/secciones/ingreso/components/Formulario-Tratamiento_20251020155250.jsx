// 🆕 Cargar establecimientos si es admin
  useEffect(() => {
    const cargarEstablecimientos = async () => {
      // ✅ Prevenir carga duplicada con ref (más robusto que state)
      ...
      if (userPayload?.rol === 'admin') {
        ...
        // ✅ Marcar ANTES de la llamada para bloquear inmediatamente
        ...
        try {
          ...
          if (response?.data) {
            // ✅ Filtrar solo activos y eliminar duplicados
            ...
            const establecimientosUnicos = Array.from(
              new Map(
                establecimientosActivos.map(est => [est.id_establecimiento, est])
              ).values()
            );
            
            console.log('🔄 Total recibido del backend:', response.data.length);
            console.log('✅ Activos:', establecimientosActivos.length);
            console.log('✅ Únicos y activos:', establecimientosUnicos.length);
            console.log('📋 Establecimientos únicos:', establecimientosUnicos);
...
const FormularioTratamiento = ({ setStep }) => {
  ...
  const { establecimientoActual, userPayload } = useSelector(state => state.auth);
  ...
  const establecimientosCargadosRef = useRef(false); // 👈 Usar ref en lugar de state
      ...
      if (userPayload?.rol === 'admin') {
        ...
        try {
          ...
          if (response?.data) {
            ...
            const establecimientosUnicos = Array.from(
              new Map(
                response.data.map(est => [est.id_establecimiento, est])
              ).values()
            );
            
            console.log('🔄 Total recibido:', response.data.length);
            console.log('✅ Únicos después de filtrar:', establecimientosUnicos.length);
            console.log('📋 Establecimientos únicos:', establecimientosUnicos);
        ...
        {userPayload?.rol === 'admin' && modoMultiple && (
          <div className='mb-6 p-4 rounded-lg border-l-4 border-orange-500 bg-orange-50'>
            ...
            <div className='text-sm text-orange-700'>
              ...
              {establecimientoActual 
                ? establecimientos.find(e => e.id_establecimiento === establecimientoActual)?.nombre_establecimiento || 'Desconocido'
          ...
          <form onSubmit={handleSubmit(onSubmit)} className='space-y-4'>
            ...
            {userPayload?.rol === 'admin' && (
              <div className='p-4 bg-orange-50 border border-orange-200 rounded-lg'>
                <label className='block text-gray-700 font-medium mb-2' htmlFor='id_establecimiento'>
                  🏢 Establecimiento *
                </label>
                ...
                <div className='mb-2 p-2 bg-gray-100 rounded text-xs'>
                  ...
                  <p>Establecimiento actual Redux: {establecimientoActual || 'Ninguno'}</p>
                </div>
                  ...
                  {establecimientos.length === 0 ? (
                    ...
                    establecimientos.map((est) => (
                      <option key={est.id_establecimiento} value={est.id_establecimiento}>
                        {est.nombre_establecimiento || est.nombre || `Establecimiento ${est.id_establecimiento}`}
                      </option>
                    ))
                  )}
                </select>
                {errors.id_establecimiento && (
                  <span className='text-red-500 text-sm'>{errors.id_establecimiento.message}</span>
                )}
                <p className='text-xs text-orange-600 mt-1'>
                  ℹ️ Como admin, debes especificar a qué establecimiento pertenece este tratamiento
                </p>
              </div>
            )}
