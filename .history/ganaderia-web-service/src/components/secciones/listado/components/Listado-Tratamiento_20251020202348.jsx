...
const ListadoTratamiento = () => {
  ...
  const eliminarTratamiento = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:3000/tratamientos/delete-tratamiento-by-id/${modalEliminar.tratamiento.id_tratamiento}`,
        {
          method: "DELETE",
          headers: {
  ...
  const abrirModalEditar = async (tratamiento) => {
    ...
    if (tratamiento.id_establecimiento) {
      try {
        ...
        const response = await obtenerTerneroHook(queryParams);
  ...
  const guardarEdicion = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:3000/tratamientos/patch-tratamiento-by-id/${modalEditar.tratamiento.id_tratamiento}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
            ...
            <div className='bg-white p-6 rounded-lg shadow-xl w-full max-w-2xl my-8'>
              ...
              <div className='flex gap-3 mt-6'>
                <button
                  ...
                  className='flex-1 bg-gray-500 hover:bg-gray-600 text-white py-2 px-4 rounded-md transition-colors disabled:opacity-50'
                >
                  Cancelar
                </button>
                <button
