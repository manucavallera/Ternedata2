...
function Navbar() {
  ...
  // Función para cargar usuarios pendientes usando el hook
  ...
  }, [userPayload?.rol, obtenerUsuariosHook]); // ✅ Dependencias
  ...
  useEffect(() => {
    if (userPayload?.rol === "admin") {
      ...
      const interval = setInterval(cargarUsuariosPendientes, 30000); // 30 segundos
      return () => clearInterval(interval);
    }
  }, [userPayload?.rol]); // ✅ Solo depende del rol, no del objeto completo
        ...
        (status !== "authenticated" && statusSession === false) ? (
          ...
          <>
            ...
            {userPayload?.rol === "admin" && (
              <li>
                <Link
                  href='/panel-admin' // Cambiado de '/admin' a '/panel-admin'
                  className='relative flex items-center gap-2 px-3 py-2 bg-white bg-opacity-20 hover:bg-opacity-30 rounded-lg transition-all cursor-pointer'
                >
