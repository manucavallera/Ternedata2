...
const Registercomponent = () => {
  ...
  // 👇 1. ESTADO PARA GUARDAR EL TOKEN (DE URL O BACKUP)
  ...
  // 👇 2. EFFECT PARA RECUPERAR EL TOKEN
  useEffect(() => {
    // A. Intentar leer de la URL
    let tokenFound = searchParams.get("token");

    // B. Si no está en URL, buscar en el "bolsillo" (LocalStorage)
    if (!tokenFound && typeof window !== "undefined") {
      tokenFound = localStorage.getItem("backupToken");
    }

    if (tokenFound) {
      console.log("🎟️ Token detectado para registro:", tokenFound);
      setInvitationToken(tokenFound);
    }
  }, [searchParams]);

  const onSubmit = handleSubmit(async (data) => {
    if (data.password === data.confirmPassword) {
      // 👇 3. USAMOS LA VARIABLE DE ESTADO
      const newUser = {
        name: data.username,
        email: data.email,
      ...
      if (res === 500) {
        ...
        // 👇 4. LIMPIEZA: Si usamos el backup, lo borramos para no ensuciar
        if (typeof window !== "undefined") {
          localStorage.removeItem("backupToken");
        }

        // 👇👇 AQUÍ ESTABA EL ERROR: CAMBIADO A /auth/login 👇👇
        setTimeout(() => {
