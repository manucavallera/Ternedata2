...
// import { setStatusRegister } from "@/store/register"; // (Parece que no se usa, lo dejé comentado)
import { useRouterSession } from "@/utils/routerSession";
import { useAuthContext } from "@/context/authContext";
import ClientOnly from "@/components/ClientOnly";
// 👇 1. IMPORTAR useSearchParams PARA LEER LA URL
...
const Registercomponent = () => {
  ...
  const onSubmit = handleSubmit(async (data) => {
    if (data.password === data.confirmPassword) {
      // 👇 3. AGREGAR EL TOKEN AL OBJETO DE REGISTRO
      const newUser = {
        name: data.username,
        email: data.email,
        password: data.password,
        invitationToken: invitationToken || null, // Si hay token, lo enviamos
      };

      console.log("Enviando registro con datos:", newUser); // Para depurar
      ...
      if (res === 500) {
        ...
        // 👇 MENSAJE INTELIGENTE: Si tenía token, le decimos que ya entró.
        const mensajeExito = invitationToken
        ...
        setTimeout(() => {
          // Si tiene token, quizás quieras loguearlo directo o mandarlo al login
        ...
        <div className='h-[calc(100vh-7rem)] flex justify-center items-center'>
          ...
          >
            <h1 className='font-bold text-2xl sm:text-3xl md:text-4xl mb-3 sm:mb-4 text-gray-700'>
              {invitationToken ? "Únete al Equipo" : "Registro"}
              {/* 👆 Título dinámico si es invitación */}
            </h1>
