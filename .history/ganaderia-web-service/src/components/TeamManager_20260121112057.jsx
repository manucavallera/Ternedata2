...
export const TeamManager = ({ establecimientoId }) => {
  ...
  const handleGenerateLink = async (e) => {
    e.preventDefault();
    setLoading(true);
    setGeneratedLink(""); // Limpiar link anterior

    try {
      console.log("🧬 Generando token para:", inviteData.email);

      // 1. Pedimos el token al Backend
      const response = await axios.get(
        `${API_URL}/auth/generar-token?email=${inviteData.email}`,
      );

      const token = response.data.token_para_copiar;

      // 2. Construimos el Link Mágico
      const origin = window.location.origin; // Detecta automáticamente localhost:3002
