...
export const TeamManager = ({ establecimientoId }) => {
  ...
  const handleInvite = async (e) => {
    ...
    try {
      // Nota: Asegúrate de que este endpoint apunta al puerto correcto (Business o Users según tu arquitectura)
      // Si invitaciones está en Business (3000), cambia API_URL a la de business
      const BUSINESS_API = "http://localhost:3000";

      await axios.post(
        `${BUSINESS_API}/api/invitaciones/crear/${establecimientoId}`,
        {
          email: inviteData.email,
          rol: inviteData.role, // Aquí enviamos "veterinario" u "operario"
        },
        {
          headers: { Authorization: `Bearer ${getToken()}` },
        },
