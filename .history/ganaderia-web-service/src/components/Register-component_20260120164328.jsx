...
const Registercomponent = () => {
  ...
  // Estado para guardar el token si viene por URL
  const [urlToken, setUrlToken] = useState(null);

  const {
    register,
    handleSubmit,
    setValue, // 👇 Importamos setValue para pre-llenar si queremos
    formState: { errors },
  } = useForm();

  const [userAlert, setuserAlert] = useState({ status: false, message: "" });

  // 1. EFFECT PARA RECUPERAR EL TOKEN DE URL (Si existiera)
  useEffect(() => {
    let tokenFound = searchParams.get("token");
    if (!tokenFound && typeof window !== "undefined") {
      tokenFound = localStorage.getItem("backupToken");
    }

    if (tokenFound) {
      console.log("🎟️ Token detectado automáticamente:", tokenFound);
      setUrlToken(tokenFound);
      // Opcional: Pre-llenar el input visual si se detecta
      setValue("invitationToken", tokenFound);
    }
  }, [searchParams, setValue]);

  const onSubmit = handleSubmit(async (data) => {
    if (data.password === data.confirmPassword) {
      // 👇 2. LOGICA HÍBRIDA: Toma el del input manual O el de la URL
      const tokenFinal = data.invitationToken || urlToken || null;

      const newUser = {
        name: data.username,
        email: data.email,
        password: data.password,
        invitationToken: tokenFinal, // 👇 Enviamos el token
      };
